...
function bb2_housekeeping($settings, $package)
{
	...
	$query = "DELETE FROM `" . $settings['log_table'] . "` WHERE `date` < DATE_SUB('" . bb2_db_date() . "', INTERVAL " . $Settings->get('auto_prune_stats') . " DAY)";
	bb2_db_query($query);

	// Waste a bunch more of the spammer's time.
...
<p>You can use this key to <a href="http://www.ioerror.us/bb2-support-key">fix this problem yourself</a>.</p>
<?php
}
